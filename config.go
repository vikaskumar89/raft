...
func (cfg *config) checkLogs(i int, m ApplyMsg) (string, bool) {
	...
	for j := 0; j < len(cfg.logs); j++ {
		// Get the log of Jth Server & check the value store at CommandIndex
		if oldValue, oldok := cfg.logs[j][m.CommandIndex]; oldok && oldValue != v {
			log.Printf("%v: log %v; server %v\n", i, cfg.logs[i], cfg.logs[j])
			// some server has already committed a different value for this entry!
			err_msg = fmt.Sprintf("commit index=%v of server=%v has command %v != server=%v has command %v",
				m.CommandIndex, i, m.Command, j, oldValue)
		}
	}
...
func (cfg *config) start1(i int, applier func(int, chan ApplyMsg)) {
	...
	if cfg.saved[i] != nil {
		...
		cfg.saved[i] = storage.MakePersister()
	}
...
func (cfg *config) checkOneLeader() int {
	for iters := 0; iters < 10; iters++ {
		...
		for i := 0; i < cfg.n; i++ {
			if cfg.connected[i] {
				//fmt.Println("Waiting for getState")
				if term, leader := cfg.rafts[i].GetState(); leader {
					leaders[term] = append(leaders[term], i)
				}
				//fmt.Println("GetState Finished")
			}
		}

		lastTermWithLeader := -1
		for term, leaders1 := range leaders {
			if len(leaders1) > 1 {
				fmt.Printf("Leaders Map %+v\n",leaders)
				cfg.t.Fatalf("term %d has %d (>1) leaders", term, len(leaders1))
			}
...
// how many servers think a log entry is committed at index?
...
// return the index of logs at which the command is supposed to be committed by the leader
// the number of servers committing the log should be >= expectedServers
func (cfg *config) one(cmd interface{}, expectedServers int, retry bool) int {
	//fmt.Printf("Sending one commit command %v\n", cmd)
	t0 := time.Now()
	starts := 0
	for time.Since(t0).Seconds() < 10 {
		// 1. find the leader & index at which it has appended the command to the log
		index := -1
		...
		if index != -1 {
			...
			for time.Since(t1).Seconds() < 2 {
				...
				if nd > 0 && nd >= expectedServers {
					//fmt.Printf("The entry for %v|%v is commited by %d servers\n", cmd, cmd1, nd)
					// committed
					if cmd1 == cmd {
			...
			//fmt.Println("No leader is elected, sleeping for 300ms")
